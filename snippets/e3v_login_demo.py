...
requests.post(watchtowerurl+'/api/cameras/action', data = {'Id': camid, 'Action': 'UPDATEMC', 'apitoken': apit}, verify=False)
print(">> Choosing {:d}: {:s} as sync source\n".format(cam['Id'], cam['Hostname']))

# List network interfaces
r = requests.get(watchtowerurl+'/api/interfaces/available', params = {'apitoken': apit}, verify=False)
j = json.loads(r.text)
...
r = requests.get(watchtowerurl+'/api/cameras/manage/configs/available', params = {'apitoken': apit}, verify=False)
j = json.loads(r.text)
...
# Connect to a camera
requests.post(watchtowerurl+'/api/cameras/action', data = {'Id': camid, 'Action': 'CONNECT', 'Iface': interface, 'Config': resolution, 'Codec': codec, 'Annotation': anno, 'Segtime': segment, 'apitoken': apit}, verify=False)
